"""Try camoufox os=windows fingerprint in the container."""
import asyncio
import sys
    ...
    async () => {
        try {
            ...
            if (!window._nvcResult && window._capInst && window._capInst.startTracelessVerification) {
                ...
                for (let t = 0; t < 60 && !window._nvcResult; t++)
                    await new Promise(r => setTimeout(r, 500));
            }
            return window._nvcResult || 'no-callback-75s';
...
async def main():
    ...
    async with AsyncCamoufox(**kwargs) as browser:
        ...
        val = await page.evaluate(CAPTCHA_VERBOSE)
        print("RESULT:", str(val)[:300], flush=True)
        await page.close()
